function params = engine_parameters()
    ...
    params.power.crank_length = 0.100;        % m (100 mm)
    params.power.rod_length = 0.400;          % m (400 mm)
    
    %% Displacer Parameters
    params.disp.crank_length = 0.120;         % m (120 mm)
    params.disp.rod_length = 0.480;           % m (480 mm)
    params.disp.volume = 0.005;               % m^3 (5000 cm^3)
    
    %% Cylinder Parameters
    params.cylinder.bore = 0.250;             % m (250 mm diameter)
    ...
    params.T_hot = 700;                       % K (hot temperature)
    params.T_cold = 350;                      % K (cold temperature)
    params.T_regenerator = (params.T_hot + params.T_cold) / 2;  % K (average)
    
    %% Pressure Conditions
    params.P_bdc = 2e6;                       % Pa (2 MPa at bottom dead center)
    params.P_atm = 101325;                    % Pa (atmospheric pressure)
    
    %% Dead Volumes
    params.V_dead_hot = 0.0002;               % m^3 (hot space dead volume)
    params.V_dead_cold = 0.0002;              % m^3 (cold space dead volume)
    params.V_regenerator = 0.0005;            % m^3 (regenerator dead volume)
    ...
    params.V_max = params.V_swept_power + params.V_swept_disp + params.V_dead_total;
    ...
    if abs(calculated_CR - params.compression_ratio) > 0.1
        warning('Specified compression ratio (%.2f) differs from calculated (%.2f)', ...
